...
**Next Action**: Pick next Ready task (B-016 load/concurrency, P6-001 dev environment, or documentation tasks)
...
| Date | What Happened |
|------|---------------|
| 2026-02-11 | **B-005 complete: Load testing setup.** Fixed expensive scenario which was broken (Q&A timed out with no extracted data). Implemented full pipeline: create patient → session → upload doc → extract → Q&A. Config: 2 VUs, 4 iterations, 8 min max, 6 min extraction timeout, P95 < 5 min threshold. Accounts for Azure OpenAI rate limiting (HTTP 429) with exponential backoff retries (observed up to 6 retries with 59s waits). Cost: ~$0.10/run. Cheap scenario unchanged (10 VUs, 30s, P95 < 500ms). Updated CLAUDE.md and load-test.sh with accurate documentation. Validation: 100% checks passed (465/465), 0% failures, P95 expensive 1m49s. **Also:** Refactored `run-e2e.sh` to require explicit flag (`--backend`, `--frontend`, or `--all`) — no default mode. Updated docs (CLAUDE.md, LOCAL_DEV.md). Increased retry jitter in `AzureRetryDefaults.cs`. |
